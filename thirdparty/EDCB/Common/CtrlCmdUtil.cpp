...
DWORD WriteVALUE( WORD ver, BYTE* buff, DWORD buffOffset, const REC_SETTING_DATA& val )
{
	...
	if( ver >= 6 ){
		pos += WriteVALUE(ver, buff, pos, val.launchViewApp);
	}
	if( ver >= 7 ){
		pos += WriteVALUE(ver, buff, pos, val.closeViewOnEnd);
	}
	WriteVALUE(0, buff, buffOffset, pos - buffOffset);
	return pos - buffOffset;
}
...
bool ReadVALUE( WORD ver, const BYTE** buff, const BYTE* buffEnd, REC_SETTING_DATA* val )
{
	...
	val->launchViewApp = 0;
	if( ver >= 6 ){
		if( !ReadVALUE(ver, &rb, buffEnd, &val->launchViewApp) ){
			return false;
		}
	}
	val->closeViewOnEnd = 0;
	if( ver >= 7 ){
		if( !ReadVALUE(ver, &rb, buffEnd, &val->closeViewOnEnd) ){
			return false;
		}
	}
	return true;
